...
function extractDestination(transcript) {
  ...
  const contextNavigation =
    /(?:return\s+to|back\s+to|switch\s+to|change\s+to|go\s+back(?:\s+to)?)\s+([a-z0-9\s]+|$)/i;
...
function useGPT(transcript) {
  // If the RegEx fails to match,
  // we can fallback to a GPT check
  try {
    console.log("Calling API...");
    // Since we're in a non-async function, we need to use .then() syntax
    fetch(
      ...
      .then((data) => {
        ...
        if (data && data.response) {
          destination = data.response.trim().toLowerCase();
    ...
    // Return null here since the actual navigation will happen in the Promise callback
    return null;
  } catch (error) {
